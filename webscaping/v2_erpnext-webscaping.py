        ...
        if full_text:
            for sub_chunk in chunk_text(full_text):
                ...
                chunks_list.append({
                    ...
                    "doc_slug": slug,
                    "category": category,
                    "tags": [category.lower(), "erpnext", "frappe", "frappe framework"],
...
if __name__ == "__main__":
    # Carregar lista de paths do arquivo JSON
    with open("path_list.json", "r", encoding="utf-8") as f:
        path_list = json.load(f)

    for path in path_list:
       # BASE_URL = f"https://docs.frappe.io/framework/user/en/{path}"
        BASE_URL = f"{path}"
        # START_PAGE = path
        START_PAGE = ""
        start_url = urljoin(BASE_URL, START_PAGE)
        print(f"\n[INICIANDO SCRAPING] Path: {path}")
        scrape_docs(start_url, limit=100000)  # Teste com 100000 páginas
